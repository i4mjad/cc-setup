...
2. **Discovery → discovery brief.** Dispatch **discovery** with the brief. It steelmans the idea and
   ...
   verdict and falsifiable kill criteria. **Gate.** On **KILL**, stop the pipeline — report the
...
3. **Scope check & phase plan.** Confirm the `<slug>` derived at Setup; all artifacts are keyed by it.
   **Assess scope by default — the user does not have to ask for this.** Scope what discovery's v1
   ...
   phase list to the user and get approval (**phase-plan gate**), then run steps 4–11 once per phase
...
- Forward & gates per `CLAUDE.md` §2. You own the human gates: the bootstrap gate (new project only),
  the phase-plan gate (large scope only — step 3), and the four pipeline gates (after discovery, after
